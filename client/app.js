// ===== STORE MODULE (API Backend) =====
const API_BASE = window.location.origin + '/api';

const Store = {
  // Cache for frequently accessed data within a render cycle
  _cache: {},
  _cacheTimeout: null,
  ...
  async getAll(key) {
    ...
    try {
      ...
      // Auto-clear cache after 2 seconds (so next render gets fresh data)
      clearTimeout(this._cacheTimeout);
      this._cacheTimeout = setTimeout(() => this.clearCache(), 2000);
      return data;
    } catch (err) {
      console.error(`Store.getAll(${key}) failed:`, err);
      return [];
    }
  },
  ...
  async get(key, id) {
    try {
      ...
      console.error(`Store.get(${key}, ${id}) failed:`, err);
      return null;
    }
  },
  ...
  async add(key, record) {
    try {
      ...
      console.error(`Store.add(${key}) failed:`, err);
      return record;
    }
  },
  ...
  async update(key, id, updates) {
    try {
      ...
      console.error(`Store.update(${key}, ${id}) failed:`, err);
      return null;
    }
  },
  ...
  async remove(key, id) {
    try {
      ...
      console.error(`Store.remove(${key}, ${id}) failed:`, err);
    }
  },
  ...
  async exportAll() {
    try {
      const res = await fetch(`${API_BASE}/_meta/export`);
      return await res.json();
    } catch (err) {
  ...
  async importAll(json) {
    try {
      await fetch(`${API_BASE}/_meta/import`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async clearCollection(key) {
    try {
      ...
      console.error(`clearCollection(${key}) failed:`, err);
    }
  }
};
...
const UI = {
  ...
  getCowName(id, cowsCache) {
    if (cowsCache) {
      const cow = cowsCache.find(c => c.id === id);
      return cow ? `${cow.name || cow.earTag} (${cow.earTag})` : 'Unknown';
    }
    return 'Unknown'; // fallback
  },
  getFacilityName(id, facilitiesCache) {
    if (facilitiesCache) {
      const f = facilitiesCache.find(fac => fac.id === id);
  ...
  async updateStorageIndicator() {
    try {
      const res = await fetch(`${API_BASE}/_meta/stats`);
      const stats = await res.json();
      const mb = (stats.dbSize / (1024 * 1024)).toFixed(2);
      const el = document.getElementById('storageIndicator');
      if (el) el.textContent = `DB: ${mb} MB (${stats.totalRecords} records)`;
    } catch(e) {
      const el = document.getElementById('storageIndicator');
      if (el) el.textContent = 'DB: Connected';
    }
  }
};
...
const Router = {
  ...
  init() {
    ...
    document.getElementById('exportBtn').addEventListener('click', async () => {
      const data = JSON.stringify(await Store.exportAll(), null, 2);
...
const Dashboard = {
  async render() {
    const [cows, pregnancies, fodderInv, facilities, milkDaily, vaccinations, diseases, healthRecords] = await Promise.all([
      Store.getAll('cm_cows'),
      Store.getAll('cm_pregnancy_tracking'),
      Store.getAll('cm_fodder_inventory'),
      Store.getAll('cm_facilities'),
      Store.getAll('cm_milk_daily'),
      Store.getAll('cm_vaccinations'),
      Store.getAll('cm_diseases'),
      Store.getAll('cm_health_records')
    ]);

    const activeCows = cows.filter(c => c.status === 'active');
    const activePregnancies = pregnancies.filter(p => p.status === 'confirmed' || p.status === 'in-progress');
    const lowStock = fodderInv.filter(i => i.quantityInStock <= i.reorderLevel);
    ...
    document.getElementById('dashboardCards').innerHTML = `
      ...
      <div class="stat-card">
        ...
        <div class="stat-info">
          ...
          <div class="stat-number">${activePregnancies.length}</div>
          <div class="stat-sub">${activePregnancies.filter(p => {
      ...
      <div class="stat-card">
        ...
        <div class="stat-info">
          <h4>Active Alerts</h4>
          <div class="stat-number">${lowStock.length + vaccinations.filter(v=>v.status!=='Completed'&&v.status!=='Cancelled'&&v.dueDate&&v.dueDate<new Date().toISOString().split('T')[0]).length + diseases.filter(d=>d.status==='Under Treatment'||d.status==='Quarantined').length}</div>
          <div class="stat-sub">${lowStock.length} low stock, ${diseases.filter(d=>d.status==='Under Treatment'||d.status==='Quarantined').length} disease cases</div>
        </div>
      </div>
    `;
    // Today's milk
    const todayMilk = milkDaily.filter(r => r.date === new Date().toISOString().split('T')[0]);
    ...
    // Recent activity
    const allRecords = [];
    const addRecords = (records, type) => records.forEach(r => allRecords.push({ ...r, _type: type }));

    const [breedingRecords, calvingRecords, weaningRecords, dewormingRecords, vetVisitRecords] = await Promise.all([
      Store.getAll('cm_breeding_records'),
      Store.getAll('cm_calving_records'),
      Store.getAll('cm_weaning_records'),
      Store.getAll('cm_deworming'),
      Store.getAll('cm_vet_visits')
    ]);

    addRecords(cows, 'Cow');
    addRecords(healthRecords, 'Health');
    addRecords(breedingRecords, 'Breeding');
    addRecords(calvingRecords, 'Calving');
    addRecords(weaningRecords, 'Weaning');
    addRecords(milkDaily, 'Milk');
    addRecords(vaccinations, 'Vaccination');
    addRecords(diseases, 'Disease');
    addRecords(dewormingRecords, 'Deworming');
    addRecords(vetVisitRecords, 'Vet Visit');
    ...
    if (allRecords.length === 0) {
      ...
      recentEl.innerHTML = allRecords.slice(0, 10).map(r => {
        ...
        else if (r._type === 'Milk') desc = `Milk recorded: ${UI.getCowName(r.cowId, cows)}`;
    ...
    let alerts = '';
    const fodderTypes = await Store.getAll('cm_fodder_types');
    lowStock.forEach(i => {
      const ft = fodderTypes.find(t => t.id === i.fodderTypeId);
...
const Locations = {
  async render() {
    const data = await Store.getAll('cm_locations');
  ...
  async openForm(id) {
    const loc = id ? await Store.get('cm_locations', id) : {};
    ...
    document.getElementById('locationForm').onsubmit = async (e) => {
      ...
      if (obj.id) await Store.update('cm_locations', obj.id, obj);
      else await Store.add('cm_locations', obj);
      UI.closeModal();
  ...
  async remove(id, name) {
    if (UI.confirmDelete(name)) {
      await Store.remove('cm_locations', id);
...
const Facilities = {
  async render() {
    const data = await Store.getAll('cm_facilities');
    const locations = await Store.getAll('cm_locations');
  ...
  async openForm(id) {
    const f = id ? await Store.get('cm_facilities', id) : {};
    const locs = (await Store.getAll('cm_locations')).map(l => ({ value: l.id, label: `${l.panchayat} - ${l.plotNumber}` }));
    ...
    `);
    document.getElementById('facilityForm').onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(e.target);
      const obj = Object.fromEntries(fd);
      ...
      if (obj.id) await Store.update('cm_facilities', obj.id, obj);
      else await Store.add('cm_facilities', obj);
      UI.closeModal();
  ...
  async showDetail(id) {
    const f = await Store.get('cm_facilities', id);
    if (!f) return;
    const loc = await Store.get('cm_locations', f.locationId);
    let feedHtml = '';
    if (f.liveFeedUrl) {
  ...
  async remove(id, name) {
    if (UI.confirmDelete(name)) {
      await Store.remove('cm_facilities', id);
      UI.closeModal();
...
const Fodder = {
  ...
  async renderTypes() {
    const data = await Store.getAll('cm_fodder_types');
  ...
  async renderInventory() {
    const data = await Store.getAll('cm_fodder_inventory');
    const fodderTypes = await Store.getAll('cm_fodder_types');
    const facilities = await Store.getAll('cm_facilities');
    ...
    tbody.innerHTML = data.map(i => {
      ...
      return `<tr>
        <td>${UI.getFacilityName(i.facilityId, facilities)}</td><td>${ft ? ft.name : '-'}</td>
  ...
  async renderSchedules() {
    const data = await Store.getAll('cm_fodder_schedules');
    const fodderTypes = await Store.getAll('cm_fodder_types');
    const facilities = await Store.getAll('cm_facilities');
    ...
    tbody.innerHTML = data.map(s => {
      const ft = fodderTypes.find(t => t.id === s.fodderTypeId);
      return `<tr>
        <td>${UI.getFacilityName(s.facilityId, facilities)}</td><td>${ft ? ft.name : '-'}</td>
  ...
  async openTypeForm(id) {
    const t = id ? await Store.get('cm_fodder_types', id) : {};
    ...
    `);
    document.getElementById('fodderTypeForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      obj.costPerUnit = parseFloat(obj.costPerUnit) || 0;
      if (obj.id) await Store.update('cm_fodder_types', obj.id, obj);
      else await Store.add('cm_fodder_types', obj);
      UI.closeModal();
      UI.showToast('Feed type saved');
      Fodder.renderTypes();
    };
  },
  async openInventoryForm(id) {
    const inv = id ? await Store.get('cm_fodder_inventory', id) : {};
    const facilities = (await Store.getAll('cm_facilities')).map(f => ({ value: f.id, label: f.shedName }));
    const types = (await Store.getAll('cm_fodder_types')).map(t => ({ value: t.id, label: t.name }));
    ...
    `);
    document.getElementById('fodderInvForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      obj.quantityInStock = parseFloat(obj.quantityInStock) || 0;
      obj.reorderLevel = parseFloat(obj.reorderLevel) || 0;
      if (obj.id) await Store.update('cm_fodder_inventory', obj.id, obj);
      else await Store.add('cm_fodder_inventory', obj);
      UI.closeModal();
      UI.showToast('Inventory saved');
      Fodder.renderInventory();
    };
  },
  async openScheduleForm(id) {
    const s = id ? await Store.get('cm_fodder_schedules', id) : {};
    const facilities = (await Store.getAll('cm_facilities')).map(f => ({ value: f.id, label: f.shedName }));
    const types = (await Store.getAll('cm_fodder_types')).map(t => ({ value: t.id, label: t.name }));
    ...
    `);
    document.getElementById('fodderSchedForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      obj.quantityPerCow = parseFloat(obj.quantityPerCow) || 0;
      if (obj.id) await Store.update('cm_fodder_schedules', obj.id, obj);
      else await Store.add('cm_fodder_schedules', obj);
      UI.closeModal();
      UI.showToast('Schedule saved');
      Fodder.renderSchedules();
    };
  },
  async removeType(id, name) { if (UI.confirmDelete(name)) { await Store.remove('cm_fodder_types', id); UI.showToast('Deleted'); Fodder.renderTypes(); } },
  async removeInventory(id) { if (confirm('Delete this inventory record?')) { await Store.remove('cm_fodder_inventory', id); UI.showToast('Deleted'); Fodder.renderInventory(); } },
  async removeSchedule(id) { if (confirm('Delete this schedule?')) { await Store.remove('cm_fodder_schedules', id); UI.showToast('Deleted'); Fodder.renderSchedules(); } }
};
...
const Cows = {
  ...
  async render(filter = '') {
    let data = await Store.getAll('cm_cows');
    if (filter) {
  ...
  async openForm(id) {
    const c = id ? await Store.get('cm_cows', id) : {};
    const facilities = (await Store.getAll('cm_facilities')).map(f => ({ value: f.id, label: f.shedName }));
    const cows = (await Store.getAll('cm_cows')).filter(cow => cow.id !== id).map(cow => ({ value: cow.id, label: `${cow.name || cow.earTag} (${cow.earTag})` }));
    ...
    const checkDuplicate = async () => {
      ...
      const allCows = await Store.getAll('cm_cows');
    ...
    document.getElementById('cowForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      ...
      if (obj.id) await Store.update('cm_cows', obj.id, obj);
      else await Store.add('cm_cows', obj);
      UI.closeModal();
  ...
  async showDetail(id) {
    const c = await Store.get('cm_cows', id);
    if (!c) return;
    const panel = document.getElementById('cowDetailPanel');
    const body = document.getElementById('cowDetailBody');
    const facility = await Store.get('cm_facilities', c.facilityId);
    const mother = c.motherId ? await Store.get('cm_cows', c.motherId) : null;
    const calves = await Store.query('cm_cows', cow => cow.motherId === id);
    const healthRecords = await Store.query('cm_health_records', h => h.cowId === id);
    const breedingRecords = await Store.query('cm_breeding_records', b => b.cowId === id);
    const idScore = Cows.getIdScore(c);
    const scoreClass = idScore >= 70 ? 'high' : idScore >= 40 ? 'medium' : 'low';

    // Verification history for this cow
    const verifications = (await Store.getAll('cm_verification_log')).filter(v => v.cowId === id);
...
const CowVerify = {
      ...
      <div class="verify-result">
        <div class="verify-cow-header">
          ...
          <div class="verify-cow-info">
            ...
            <p>Facility: ${cow.facilityId ? UI.getFacilityName(cow.facilityId, facilities) : '-'}</p>
          </div>
        </div>
  ...
  async submitVerification(cowId, totalChecks) {
    ...
    const cow = await Store.get('cm_cows', cowId);
    const logEntry = {
      cowId,
      earTag: cow?.earTag || '',
    ...
    await Store.add('cm_verification_log', logEntry);
  ...
  async renderLog() {
    ...
    if (!tbody) return;
    const data = await Store.getAll('cm_verification_log');
    const cows = await Store.getAll('cm_cows');
    if (data.length === 0) {
  ...
  async removeLog(id) { if (confirm('Delete this verification record?')) { await Store.remove('cm_verification_log', id); UI.showToast('Deleted'); CowVerify.renderLog(); } },
  async clearLog() { if (confirm('Clear entire verification log?')) { await Store.clearCollection('cm_verification_log'); UI.showToast('Log cleared'); CowVerify.renderLog(); } }
};

// ===== HEALTH =====
const Health = {
  async render() {
    const records = await Store.getAll('cm_health_records');
    const cows = await Store.getAll('cm_cows');
    const facilities = await Store.getAll('cm_facilities');
    ...
    if (individual.length === 0) {
      ...
      indTbody.innerHTML = individual.sort((a,b) => new Date(b.date) - new Date(a.date)).map(h => `<tr>
        <td>${UI.formatDate(h.date)}</td><td>${UI.getCowName(h.cowId, cows)}</td>
    ...
    if (group.length === 0) {
      ...
      grpTbody.innerHTML = group.sort((a,b) => new Date(b.date) - new Date(a.date)).map(h => `<tr>
        <td>${UI.formatDate(h.date)}</td><td>${UI.getFacilityName(h.facilityId, facilities)}</td>
  ...
  async openForm(id) {
    const h = id ? await Store.get('cm_health_records', id) : {};
    const cows = (await Store.getAll('cm_cows')).map(c => ({ value: c.id, label: `${c.name || c.earTag} (${c.earTag})` }));
    const facilities = (await Store.getAll('cm_facilities')).map(f => ({ value: f.id, label: f.shedName }));
    ...
    document.getElementById('healthForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      if (obj.id) await Store.update('cm_health_records', obj.id, obj);
      else await Store.add('cm_health_records', obj);
      UI.closeModal();
  ...
  async remove(id) {
    if (confirm('Delete this health record?')) {
      await Store.remove('cm_health_records', id);
...
const Breeding = {
  async render() {
    await this.renderBreeding();
    await this.renderPregnancy();
  },
  async renderBreeding() {
    const data = (await Store.getAll('cm_breeding_records')).sort((a,b) => new Date(b.serviceDate) - new Date(a.serviceDate));
    const cows = await Store.getAll('cm_cows');
    ...
    tbody.innerHTML = data.map(b => {
      ...
      return `<tr>
        <td>${UI.formatDate(b.serviceDate)}</td><td>${UI.getCowName(b.cowId, cows)}</td>
  ...
  async renderPregnancy() {
    const data = await Store.getAll('cm_pregnancy_tracking');
    const cows = await Store.getAll('cm_cows');
  ...
  async openBreedingForm(id) {
    const b = id ? await Store.get('cm_breeding_records', id) : {};
    const cows = (await Store.getAll('cm_cows')).filter(c => c.sex === 'female' && c.status === 'active').map(c => ({ value: c.id, label: `${c.name || c.earTag} (${c.earTag})` }));
    ...
    `);
    document.getElementById('breedingForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      if (obj.wasSuccessful === '') obj.wasSuccessful = null;
      if (obj.id) await Store.update('cm_breeding_records', obj.id, obj);
      else {
        const record = await Store.add('cm_breeding_records', obj);
        ...
        if (obj.wasSuccessful === 'true') {
          ...
          await Store.add('cm_pregnancy_tracking', {
            cowId: obj.cowId,
  ...
  async openPregnancyForm(id) {
    const p = id ? await Store.get('cm_pregnancy_tracking', id) : {};
    const cows = (await Store.getAll('cm_cows')).filter(c => c.sex === 'female').map(c => ({ value: c.id, label: `${c.name || c.earTag} (${c.earTag})` }));
    const allCowsCache = await Store.getAll('cm_cows');
    const breedings = (await Store.getAll('cm_breeding_records')).map(b => ({ value: b.id, label: `${UI.getCowName(b.cowId, allCowsCache)} - ${UI.formatDate(b.serviceDate)}` }));
    ...
    document.getElementById('pregnancyForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      obj.checkups = id ? ((await Store.get('cm_pregnancy_tracking', id))?.checkups || []) : [];
      if (obj.id) await Store.update('cm_pregnancy_tracking', obj.id, obj);
      else await Store.add('cm_pregnancy_tracking', obj);
      UI.closeModal();
      UI.showToast('Pregnancy record saved');
      Breeding.renderPregnancy();
    };
  },
  async addCheckup(pregnancyId) {
    ...
    document.getElementById('checkupForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      const pregnancy = await Store.get('cm_pregnancy_tracking', pregnancyId);
      if (pregnancy) {
        const checkups = pregnancy.checkups || [];
        checkups.push(obj);
        await Store.update('cm_pregnancy_tracking', pregnancyId, { checkups, status: 'in-progress' });
      }
      UI.closeModal();
  ...
  async removeBreeding(id) {
    if (confirm('Delete this breeding record?')) {
      await Store.remove('cm_breeding_records', id);
      UI.showToast('Deleted');
      Breeding.renderBreeding();
    }
  },
  async removePregnancy(id) {
    if (confirm('Delete this pregnancy record?')) {
      await Store.remove('cm_pregnancy_tracking', id);
      UI.showToast('Deleted');
      Breeding.renderPregnancy();
    }
  }
...
const Calving = {
  async render() {
    const data = (await Store.getAll('cm_calving_records')).sort((a,b) => new Date(b.calvingDate) - new Date(a.calvingDate));
    const cows = await Store.getAll('cm_cows');
    ...
    tbody.innerHTML = data.map(c => {
      const calves = (c.calves || []).map(calf => {
        const calfCow = cows.find(cow => cow.id === calf.calfId);
        return `${calfCow ? (calfCow.name || calfCow.earTag) : 'Unknown'} (${calf.sex}, ${calf.birthWeight}kg)`;
      }).join(', ');
      return `<tr>
        <td>${UI.formatDate(c.calvingDate)}</td><td>${UI.getCowName(c.motherId, cows)}</td>
  ...
  async openForm(id) {
    const c = id ? await Store.get('cm_calving_records', id) : {};
    const pregnantCows = (await Store.getAll('cm_cows')).filter(cow => cow.sex === 'female' && cow.status === 'active').map(cow => ({ value: cow.id, label: `${cow.name || cow.earTag} (${cow.earTag})` }));
    ...
    `);
    document.getElementById('calvingForm').onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(e.target);
      const obj = {
      ...
      if (obj.id) {
        await Store.update('cm_calving_records', obj.id, obj);
      } else {
        // Create calf cow records
        const mother = await Store.get('cm_cows', obj.motherId);
        const calvesWithIds = [];
        for (const calf of obj.calves) {
          const newCow = await Store.add('cm_cows', {
          ...
          calvesWithIds.push({ ...calf, calfId: newCow.id });
        }
        obj.calves = calvesWithIds;
        await Store.add('cm_calving_records', obj);
        // Update pregnancy status if exists
        const pregnancies = await Store.query('cm_pregnancy_tracking', p => p.cowId === obj.motherId && (p.status === 'confirmed' || p.status === 'in-progress'));
        for (const p of pregnancies) {
          await Store.update('cm_pregnancy_tracking', p.id, { status: 'delivered' });
        }
      }
      UI.closeModal();
  ...
  async remove(id) {
    if (confirm('Delete this calving record?')) {
      await Store.remove('cm_calving_records', id);
      UI.showToast('Deleted');
      Calving.render();
    }
  }
...
const Weaning = {
  async render() {
    const data = (await Store.getAll('cm_weaning_records')).sort((a,b) => new Date(b.weaningDate) - new Date(a.weaningDate));
    const cows = await Store.getAll('cm_cows');
    ...
    tbody.innerHTML = data.map(w => `<tr>
      <td>${UI.getCowName(w.calfId, cows)}</td><td>${UI.formatDate(w.weaningDate)}</td>
  ...
  async openForm(id) {
    const w = id ? await Store.get('cm_weaning_records', id) : {};
    const calves = (await Store.getAll('cm_cows')).filter(c => c.motherId).map(c => ({ value: c.id, label: `${c.name || c.earTag} (${c.earTag})` }));
    ...
    calfSelect.addEventListener('change', async () => {
      const calf = await Store.get('cm_cows', calfSelect.value);
      if (calf) {
        const calvingRecords = await Store.getAll('cm_calving_records');
  ...
  async calcMetrics() {
    ...
    const calf = await Store.get('cm_cows', calfId);
  ...
  async remove(id) {
    if (confirm('Delete this weaning record?')) {
      await Store.remove('cm_weaning_records', id);
      UI.showToast('Deleted');
      Weaning.render();
    }
  }
...
const MilkProduction = {
  async render() {
    ...
    await this.renderLactation();
    await this.renderSales();
  },
  async renderDaily() {
    const data = await Store.getAll('cm_milk_daily');
    const cows = await Store.getAll('cm_cows');
    ...
    tbody.innerHTML = sorted.map(r => {
      ...
      return `<tr>
        ...
        <td>${UI.getCowName(r.cowId, cows)}</td>
  ...
  async renderLactation() {
    const data = await Store.getAll('cm_milk_lactation');
    const cows = await Store.getAll('cm_cows');
    const milkDaily = await Store.getAll('cm_milk_daily');
    ...
    grid.innerHTML = data.map(l => {
      const cow = cows.find(c => c.id === l.cowId);
      ...
      const dailyRecords = milkDaily.filter(d =>
  ...
  async renderSales() {
    const data = await Store.getAll('cm_milk_sales');
  ...
  async openDailyForm(id) {
    const r = id ? await Store.get('cm_milk_daily', id) : {};
    const cows = (await Store.getAll('cm_cows')).filter(c => c.status === 'active' && c.sex !== 'Male');
    ...
    `);
    document.getElementById('milkDailyForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      if (obj.id) await Store.update('cm_milk_daily', obj.id, obj);
      else await Store.add('cm_milk_daily', obj);
      UI.closeModal(); UI.showToast('Milk record saved');
      MilkProduction.render();
    };
  },
  async openLactationForm(id) {
    const l = id ? await Store.get('cm_milk_lactation', id) : {};
    const cows = (await Store.getAll('cm_cows')).filter(c => c.sex !== 'Male');
    ...
    `);
    document.getElementById('lactationForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      if (obj.id) await Store.update('cm_milk_lactation', obj.id, obj);
      else await Store.add('cm_milk_lactation', obj);
      UI.closeModal(); UI.showToast('Lactation record saved');
      MilkProduction.render();
    };
  },
  async openSalesForm(id) {
    const r = id ? await Store.get('cm_milk_sales', id) : {};
    ...
    document.getElementById('milkSalesForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      if (obj.id) await Store.update('cm_milk_sales', obj.id, obj);
      else await Store.add('cm_milk_sales', obj);
      UI.closeModal(); UI.showToast('Sale recorded');
      MilkProduction.render();
    };
  },
  async removeDaily(id) { if (confirm('Delete this milk record?')) { await Store.remove('cm_milk_daily', id); UI.showToast('Deleted'); MilkProduction.render(); } },
  async removeLactation(id) { if (confirm('Delete this lactation record?')) { await Store.remove('cm_milk_lactation', id); UI.showToast('Deleted'); MilkProduction.render(); } },
  async removeSale(id) { if (confirm('Delete this sale record?')) { await Store.remove('cm_milk_sales', id); UI.showToast('Deleted'); MilkProduction.render(); } }
};
...
const Vaccination = {
  ...
  async renderUpcoming() {
    const data = await Store.getAll('cm_vaccinations');
    const cows = await Store.getAll('cm_cows');
    const facilities = await Store.getAll('cm_facilities');
    const today = new Date().toISOString().split('T')[0];
    ...
    grid.innerHTML = upcoming.sort((a, b) => (a.dueDate||'').localeCompare(b.dueDate||'')).map(v => {
      ...
      const target = v.targetType === 'Individual' ? UI.getCowName(v.cowId, cows) : (v.facilityId ? UI.getFacilityName(v.facilityId, facilities) : v.groupName || 'Group');
  ...
  async renderCompleted() {
    const data = (await Store.getAll('cm_vaccinations')).filter(v => v.status === 'Completed');
    const cows = await Store.getAll('cm_cows');
    ...
    tbody.innerHTML = [...data].sort((a, b) => (b.dateGiven||'').localeCompare(a.dateGiven||'')).map(v => {
      const target = v.targetType === 'Individual' ? UI.getCowName(v.cowId, cows) : (v.groupName || 'Group');
      return `<tr>
  ...
  async openForm(id) {
    const v = id ? await Store.get('cm_vaccinations', id) : {};
    const cows = await Store.getAll('cm_cows');
    const facilities = await Store.getAll('cm_facilities');
    ...
    document.getElementById('vaccForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      if (obj.id) await Store.update('cm_vaccinations', obj.id, obj);
      else await Store.add('cm_vaccinations', obj);
      UI.closeModal(); UI.showToast('Vaccination saved');
      Vaccination.render();
    };
  },
  async markComplete(id) {
    const v = await Store.get('cm_vaccinations', id);
    if (!v) return;
    await Store.update('cm_vaccinations', id, { status: 'Completed', dateGiven: new Date().toISOString().split('T')[0] });
    UI.showToast('Marked as completed');
    Vaccination.render();
  },
  async remove(id) { if (confirm('Delete this vaccination record?')) { await Store.remove('cm_vaccinations', id); UI.showToast('Deleted'); Vaccination.render(); } }
};

// ===== DISEASE TRACKING =====
const DiseaseTracking = {
  async render() {
    const data = await Store.getAll('cm_diseases');
    const cows = await Store.getAll('cm_cows');
    ...
    tbody.innerHTML = [...data].sort((a, b) => (b.dateReported||'').localeCompare(a.dateReported||'')).map(d => {
      ...
      return `<tr>
        ...
        <td>${UI.getCowName(d.cowId, cows)}</td>
  ...
  async openForm(id) {
    const d = id ? await Store.get('cm_diseases', id) : {};
    const cows = await Store.getAll('cm_cows');
    ...
    document.getElementById('diseaseForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      if (obj.id) await Store.update('cm_diseases', obj.id, obj);
      else await Store.add('cm_diseases', obj);
      UI.closeModal(); UI.showToast('Disease record saved');
      DiseaseTracking.render();
    };
  },
  async remove(id) { if (confirm('Delete this disease record?')) { await Store.remove('cm_diseases', id); UI.showToast('Deleted'); DiseaseTracking.render(); } }
};

// ===== DEWORMING =====
const Deworming = {
  async render() {
    const data = await Store.getAll('cm_deworming');
    const cows = await Store.getAll('cm_cows');
    ...
    tbody.innerHTML = [...data].sort((a, b) => (b.date||'').localeCompare(a.date||'')).map(d => {
      const target = d.targetType === 'Individual' ? UI.getCowName(d.cowId, cows) : (d.groupName || 'Group');
  ...
  async openForm(id) {
    const d = id ? await Store.get('cm_deworming', id) : {};
    const cows = await Store.getAll('cm_cows');
    ...
    document.getElementById('dewormForm').onsubmit = async (e) => {
      e.preventDefault();
      const obj = Object.fromEntries(new FormData(e.target));
      if (obj.id) await Store.update('cm_deworming', obj.id, obj);
      else await Store.add('cm_deworming', obj);
      UI.closeModal(); UI.showToast('Deworming record saved');
      Deworming.render();
    };
  },
  async remove(id) { if (confirm('Delete this deworming record?')) { await Store.remove('cm_deworming', id); UI.showToast('Deleted'); Deworming.render(); } }
};

// ===== VET VISITS =====
const VetVisits = {
  async render() {
    const data = await Store.getAll('cm_vet_visits');
    const cows = await Store.getAll('cm_cows');
    ...
    tbody.innerHTML = [...data].sort((a, b) => (b.visitDate||'').localeCompare(a.visitDate||'')).map(v => {
      const cowNames = (v.cowIds || []).map(id => UI.getCowName(id, cows)).join(', ') || v.cowsSeen || '-';
      return `<tr>
  ...
  async openForm(id) {
    const v = id ? await Store.get('cm_vet_visits', id) : {};
    const cows = await Store.getAll('cm_cows');
    ...
    `);
    document.getElementById('vetVisitForm').onsubmit = async (e) => {
      e.preventDefault();
      const fd = new FormData(e.target);
      const obj = Object.fromEntries(fd);
      ...
      if (obj.id) await Store.update('cm_vet_visits', obj.id, obj);
      else await Store.add('cm_vet_visits', obj);
      UI.closeModal(); UI.showToast('Vet visit recorded');
      VetVisits.render();
    };
  },
  async remove(id) { if (confirm('Delete this vet visit record?')) { await Store.remove('cm_vet_visits', id); UI.showToast('Deleted'); VetVisits.render(); } }
};
...
const Alerts = {
  ...
  async refresh() {
    ...
    const [vaccinations, cows, facilities, dewormings, pregnancies, activeDiseases_raw, fodderInv, fodderTypes, vetVisits, healthRecords] = await Promise.all([
      Store.getAll('cm_vaccinations'),
      Store.getAll('cm_cows'),
      Store.getAll('cm_facilities'),
      Store.getAll('cm_deworming'),
      ...
      Store.getAll('cm_diseases'),
      Store.getAll('cm_fodder_inventory'),
      Store.getAll('cm_fodder_types'),
      Store.getAll('cm_vet_visits'),
      Store.getAll('cm_health_records')
    ]);

    // 1. Vaccination alerts
    const pendingVacc = vaccinations.filter(v => v.status !== 'Completed' && v.status !== 'Cancelled');
    const overdueVacc = pendingVacc.filter(v => v.dueDate && v.dueDate < today);
    const soonVacc = pendingVacc.filter(v => v.dueDate && v.dueDate >= today && UI.daysBetween(today, v.dueDate) <= 7);
    ...
    // 2. Deworming alerts
    const dewormingsWithDue = dewormings.filter(d => d.nextDueDate);
    const overdueDeworm = dewormingsWithDue.filter(d => d.nextDueDate < today);
    const soonDeworm = dewormingsWithDue.filter(d => d.nextDueDate >= today && UI.daysBetween(today, d.nextDueDate) <= 7);
    ...
    const activeDiseases = activeDiseases_raw.filter(d => d.status === 'Under Treatment' || d.status === 'Quarantined');
    if (activeDiseases.length) {
      let rows = '';
      activeDiseases.forEach(d => {
        rows += `<div class="alert-row"><span class="alert-icon">&#x1F6A8;</span><span class="alert-text"><strong>${d.diseaseName}</strong> - ${UI.getCowName(d.cowId, cows)} (${d.status})</span><span class="alert-date">${UI.formatDate(d.dateReported)}</span></div>`;
      });
      sections.push({ title: 'Active Disease Cases', count: activeDiseases.length, countClass: '', rows });
    }

    // 5. Fodder low stock
    const lowStock = fodderInv.filter(i => parseFloat(i.quantityInStock) <= parseFloat(i.reorderLevel));
    if (lowStock.length) {
      let rows = '';
      lowStock.forEach(i => {
        const ft = fodderTypes.find(t => t.id === i.fodderTypeId);
    ...
    // 6. Vet follow-ups
    const vetWithFollowup = vetVisits.filter(v => v.followUpDate);
    const overdueFollowups = vetWithFollowup.filter(v => v.followUpDate <= today);
    const soonFollowups = vetWithFollowup.filter(v => v.followUpDate > today && UI.daysBetween(today, v.followUpDate) <= 7);
    ...
    const healthWithFollowup = healthRecords.filter(h => h.nextFollowUp);
    const overdueHealth = healthWithFollowup.filter(h => h.nextFollowUp <= today);
...
document.addEventListener('submit', async (e) => {
  if (e.target.id === 'weaningForm') {
    ...
    const calf = await Store.get('cm_cows', obj.calfId);
    ...
    if (obj.id) await Store.update('cm_weaning_records', obj.id, obj);
    else await Store.add('cm_weaning_records', obj);
    UI.closeModal();
